...
void MyBoundingObject::Init(void)
{
	m_fRadius = 0.0f;
...
void MyBoundingObject::Swap(MyBoundingObject& other)
{
...
void MyBoundingObject::Release(void)
{

}
//The big 3
MyBoundingObject::MyBoundingObject(std::vector<vector3> a_lVectorList)
{
...
MyBoundingObject::MyBoundingObject(MyBoundingObject const& other)
{
...
MyBoundingObject& MyBoundingObject::operator=(MyBoundingObject const& other)
{
	if (this != &other)
	{
		Release();
		Init();
		MyBoundingObject temp(other);
		Swap(temp);
	}
	return *this;
}
MyBoundingObject::~MyBoundingObject() { Release(); };
//Accessors
void MyBoundingObject::SetModelMatrix(matrix4 a_m4ToWorld) { m_m4ToWorld = a_m4ToWorld; }
matrix4 MyBoundingObject::GetModelMatrix(void) { return m_m4ToWorld; }
vector3 MyBoundingObject::GetCenterG(void) { return vector3(m_m4ToWorld * vector4(m_v3Center, 1.0f)); }
float MyBoundingObject::GetRadius(void) { return m_fRadius; }
vector3 MyBoundingObject::GetSize(void) { return m_v3Size; }
vector3 MyBoundingObject::GetMax(void) { return m_v3Max; }
vector3 MyBoundingObject::GetMin(void) { return m_v3Min; }
void MyBoundingObject::SetColorSphere(vector3 color) { m_v3ColorSphere = color;  }
void MyBoundingObject::SetColorCube(vector3 color) { m_v3ColorCube = color; }
//--- Non Standard Singleton Methods
bool MyBoundingObject::IsCollidingCube(MyBoundingObject* const a_pOther)
{
...
bool MyBoundingObject::IsCollidingSphere(MyBoundingObject* const a_pOther)
{
	//Collision check goes here
	vector3 v3Temp = vector3(m_m4ToWorld * vector4(m_v3Center, 1.0f));
...
void MyBoundingObject::DrawBO(MeshManagerSingleton* m_pMeshMngr)
{
	m_pMeshMngr->AddSphereToQueue(glm::translate(vector3(GetCenterG())) * glm::scale(vector3(GetRadius() * 2.0f)), m_v3ColorSphere, WIRE);
	m_pMeshMngr->AddCubeToQueue(glm::translate(vector3(GetCenterG())) * glm::scale(vector3(GetSize())), m_v3ColorCube, WIRE);
}
